...
namespace Platinio.TweenEngine
{
    ...
    public class PlatinioTween : Singleton<PlatinioTween>
    {
        ...
        {
            var tween = TweenPool.GetVector3Tween(t.localScale, to, time);
        ...
        {
            var time = Vector3.Distance(t.position, to) / speed;
        ...
        {
            var time = Vector3.Distance(go.transform.position, to) / speed;
        ...
        {
            var time = Vector3.Distance(rect.transform.position, to) / speed;
        ...
        {
            var time = Math.Abs(obj.localScale.x - value) / speed;
            return ScaleX(obj, value, time);
        }

        public BaseTween ScaleXAtSpeed(GameObject obj, float value, float speed)
        {
            var time = Math.Abs(obj.transform.localScale.x - value) / speed;
            return ScaleX(obj, value, time);
        }
        ...
        {
            var time = Math.Abs(obj.localScale.y - value) / speed;
            return ScaleY(obj, value, time);
        }

        public BaseTween ScaleYAtSpeed(GameObject obj, float value, float speed)
        {
            var time = Math.Abs(obj.transform.localScale.y - value) / speed;
            return ScaleY(obj, value, time);
        }
        ...
        {
            var time = Math.Abs(obj.localScale.z - value) / speed;
            return ScaleZ(obj, value, time);
        }

        public BaseTween ScaleZAtSpeed(GameObject obj, float value, float speed)
        {
            var time = Math.Abs(obj.transform.localScale.z - value) / speed;
            return ScaleZ(obj, value, time);
        }
        ...
        {
            var tween = TweenPool.GetQuaternionTween(t.rotation, to, time);
        ...
        {
            var t = Mathf.Abs(cg.alpha - 1.0f) / speed;
            return Fade(cg, 1.0f, t);
        }

        public BaseTween Fade(CanvasGroup cg, float to, float t)
        {
            var tween = TweenPool.GetValueTween(cg.alpha, to, t);
        ...
        {
            var t = Math.Abs(cg.alpha - to) / speed;
            return Fade(cg, to, t);
        }

        public BaseTween Fade(Image image, float to, float t)
        {
            var tween = TweenPool.GetValueTween(image.color.a, to, t);
        ...
        {
            var tween = TweenPool.GetValueTween(material.GetFloat(propertyHash), value, t);
        ...
        {
            var t = Math.Abs(img.color.a - to) / speed;
        ...
        {
            var t = Mathf.Abs(img.color.a - 1.0f) / speed;
            return Fade(img, 1.0f, t);
        }

        public BaseTween Fade(SpriteRenderer sprite, float to, float t)
        {
            var tween = TweenPool.GetValueTween(sprite.color.a, to, t);
            tween.SetOnUpdateFloat(delegate (float v)
            {
                if (sprite == null)
                {
        ...
        {
            var t = Math.Abs(sprite.color.a - to) / speed;
        ...
        {
            var t = Mathf.Abs(sprite.color.a - 1.0f) / speed;
            return Fade(sprite, 1.0f, t);
        }
        ...
        {
            var tween = TweenPool.GetColorTween(sprite.color, to, t);
        ...
        {
            var t = CalculateColorDistance(sprite.color, to) / speed;
            return ColorTween(sprite, to, t);
        }

        public BaseTween ColorTween(Image image, Color to, float t)
        {
            var tween = TweenPool.GetColorTween(image.color, to, t);
            tween.SetOnUpdateColor(delegate (Color c)
            {
                if (image == null)
                {
        ...
        {
            var tween = ValueTween(img.fillAmount, to, t);
            tween.SetOnUpdateFloat( delegate (float v)
            {
                if (img != null)                
                    img.fillAmount = v;               
        ...
        {
            var tween = ValueTweenAtSpeed(img.fillAmount, to, speed);
            tween.SetOnUpdateFloat(delegate (float v)
            {
                if (img != null)
                    img.fillAmount = v;
        ...
        {
            var tween = TweenPool.GetMoveTween(obj, to, t);
            return ProcessTween(tween);
        }

        public BaseTween MoveAtSpeed(Transform obj, Transform to, float speed)
        {
            var t = Vector3.Distance(obj.position, to.position) / speed;
            return Move(obj, to, t);
        }

        public BaseTween Move(Transform obj, Vector3 to, float t)
        {
            var tween = TweenPool.GetVector3Tween(obj.position, to, t);
        ...
        {
            var t = Vector3.Distance(obj.position, to) / speed;
            return Move(obj, to, t);
        }
        ...
        {
            var t = Vector3.Distance(obj.transform.position, to.position) / speed;
            return Move(obj, to, t);
        }
        ...
        {
            var t = Vector3.Distance(obj.transform.position, to) / speed;
            return Move(obj, to, t);
        }
        ...
        {
            var t = Vector3.Distance(obj.transform.position, to.transform.position) / speed;
            return Move(obj, to, t);
        }
        ...
        {
            var t = Vector3.Distance(obj.position, to.transform.position) / speed;
            return Move(obj, to, t);
        }
        ...
        {
            var t = Vector2.Distance(new Vector2(rect.anchoredPosition.x, rect.anchoredPosition.y), new Vector2(rect.anchoredPosition.x, rect.anchoredPosition.y)) / speed;
        ...
        {
            var pos = rect.FromAbsolutePositionToAnchoredPosition(absolutePosition, canvas, pivotPreset);
        ...
        public BaseTween MoveUIAtSpeed(RectTransform rect, Vector2 absolutePosition, RectTransform canvas, float speed, PivotPreset pivotPreset = PivotPreset.MiddleCenter)
        {
            ...
            var time = Vector3.Distance(rect.anchoredPosition, pos) / speed;
        ...
        {
            var pos = rect.FromAnchoredPositionToAbsolutePosition(canvas, pivotPreset);
            var finalPos = pos + translation;
