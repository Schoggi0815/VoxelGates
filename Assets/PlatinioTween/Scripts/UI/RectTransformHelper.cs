...
namespace Platinio.UI
{
    ...
    public static class RectTransformHelper
    {
        ...
        {            
            var centerAnchor = ( rect.anchorMax + rect.anchorMin ) * 0.5f;          
        ...
        {
            var canvas = RequestCanvas();

            var size = rect.rect.size;
            var deltaPivot = rect.pivot - pivot;

            //Vector2 deltaPosition = new Vector2(deltaPivot.x * size.x, deltaPivot.y * size.y);
            var deltaPosition = new Vector2( rect.right.x , rect.right.y ).normalized * rect.rect.size.x * deltaPivot.x;
